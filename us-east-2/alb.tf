...
module "alb" {
  ...
  source  = "app.terraform.io/geo-fort/alb/aws"
  version = "1.0.0"
  ...
  https_listeners = [
    {
      port            = 443
      protocol        = "HTTPS"
      certificate_arn = data.aws_acm_certificate.karl_ssl_cert.arn
    },
  ]
  ...
  https_listener_rules = [
    {
      ...
      conditions = [{
        ...
        host_headers = ["www.${data.aws_route53_zone.geo-fort_zone.name}"]
      }]
    },
  ]
